...
<template>
  <StudentLayout title="Certificate Verification — SPI E-Learning Platform">
    <div class="space-y-6 max-w-7xl mx-auto pb-12 print:p-0 print:m-0">
      ...
      <!-- FEEDBACK ALERTS -->
      <div v-if="downloadFeedback" class="p-3 bg-emerald-500/10 border border-emerald-500/30 rounded-2xl text-xs text-emerald-300 font-bold flex items-center gap-2 print:hidden animate-in fade-in">
        <span>✅</span>
        <span>{{ downloadFeedback }}</span>
      </div>
      <div v-if="copyFeedback" class="p-3 bg-purple-500/10 border border-purple-500/30 rounded-2xl text-xs text-purple-300 font-bold flex items-center gap-2 print:hidden animate-in fade-in">
        <span>📋</span>
      ...
      <div class="grid grid-cols-1 lg:grid-cols-12 gap-6">
        ...
        <div class="lg:col-span-8 space-y-6">
          ...
          <div class="bg-[#0F172A]/90 border border-slate-800/80 rounded-3xl p-5 sm:p-6 shadow-xl space-y-5 print:hidden">
            
            <!-- 3 Tabs -->
            <div class="flex items-center gap-2 border-b border-slate-800/80 pb-3 overflow-x-auto">
              <button
                @click="activeTab = 'id'"
                :class="[
                  activeTab === 'id'
            ...
            <div v-if="activeTab === 'id'" class="space-y-3">
              <div>
                <h3 class="text-sm font-bold text-white">Enter Certificate ID</h3>
                <p class="text-xs text-slate-400 mt-0.5">Enter the certificate ID exactly as it appears on the certificate</p>
              </div>
              ...
              <div class="flex flex-col sm:flex-row items-center gap-3 pt-1">
                <div class="relative flex-1 w-full">
                  <input
                    ...
                    class="w-full bg-slate-900 border border-slate-700/80 rounded-2xl px-4 py-2.5 text-xs text-white font-mono placeholder-slate-500 focus:outline-none focus:border-purple-500 transition-colors"
                  />
                </div>
                ...
                <button
                  ...
                  :disabled="isLoading"
                  class="w-full sm:w-auto px-6 py-2.5 rounded-2xl bg-gradient-to-r from-purple-600 to-indigo-600 hover:from-purple-500 hover:to-indigo-500 text-white font-bold text-xs shadow-lg shadow-purple-950/50 flex items-center justify-center gap-2 transition-all cursor-pointer active:scale-95 disabled:opacity-50"
                >
            ...
            <div v-else-if="activeTab === 'link'" class="space-y-3">
              <div>
                <h3 class="text-sm font-bold text-white">Paste Certificate Verification Link</h3>
                <p class="text-xs text-slate-400 mt-0.5">Enter the full verification URL from the certificate</p>
              </div>
            ...
            <div v-else-if="activeTab === 'qr'" class="space-y-4">
              <div>
                <h3 class="text-sm font-bold text-white">Scan Certificate QR Code</h3>
                <p class="text-xs text-slate-400 mt-0.5">Upload or scan the QR code from a certificate to verify its authenticity</p>
              </div>

              <div class="border-2 border-dashed border-slate-700/80 hover:border-purple-500/50 rounded-2xl p-6 text-center space-y-3 bg-slate-900/50 transition-colors">
                <div class="w-12 h-12 rounded-2xl bg-purple-600/20 border border-purple-500/30 text-purple-300 flex items-center justify-center text-xl mx-auto">
                  📷
                </div>
                <div>
                  <p class="text-xs font-bold text-white">Upload Certificate QR Image</p>
                  <p class="text-[11px] text-slate-400 mt-0.5">Supports PNG, JPG, or PDF snippets</p>
                </div>
                ...
                <div class="flex items-center justify-center gap-3 pt-1">
                  ...
                  <button
                    ...
                    class="px-4 py-2 rounded-xl bg-slate-800 hover:bg-slate-700 text-slate-200 font-bold text-xs border border-slate-700 transition-colors"
                  >
          ...
          <!-- LOADING STATE -->
          <div v-if="isLoading" class="bg-[#0F172A]/90 border border-purple-500/40 rounded-3xl p-10 text-center space-y-3 shadow-xl">
            <div class="w-12 h-12 rounded-full border-4 border-purple-500/20 border-t-purple-500 animate-spin mx-auto"></div>
            <h3 class="text-base font-bold text-white">Verifying Certificate Authenticity...</h3>
            <p class="text-xs text-slate-400">Querying official blockchain records and registry database</p>
          </div>

          <!-- ERROR STATE: NOT FOUND / INVALID -->
          <div v-else-if="currentResult.status === 'not_found' || currentResult.status === 'invalid' || currentResult.status === 'revoked'" class="bg-[#0F172A]/90 border border-rose-500/40 rounded-3xl p-6 shadow-2xl space-y-4">
            <div class="flex items-start gap-4">
              <div class="w-12 h-12 rounded-2xl bg-rose-600/20 border border-rose-500/40 text-rose-400 flex items-center justify-center text-2xl shrink-0">
                ⚠️
              </div>
              <div class="space-y-1">
            ...
            <div class="p-4 bg-slate-950/80 rounded-2xl border border-slate-800 text-xs text-slate-400 space-y-1">
              <p class="font-bold text-white">Possible Reasons:</p>
            ...
            <div class="flex items-center gap-3 pt-1">
              <button
                @click="resetSearch"
                class="px-5 py-2.5 rounded-xl bg-purple-600 hover:bg-purple-500 text-white font-bold text-xs shadow-md"
              >
                Try Sample Specimen
              </button>
              <a
                href="mailto:support@spilms.tech"
                class="px-5 py-2.5 rounded-xl bg-slate-800 hover:bg-slate-700 text-slate-200 font-bold text-xs border border-slate-700"
              >
                Contact Support
              </a>
            </div>
          ...
          <div v-else class="space-y-4">
            ...
            <div class="flex items-center justify-between print:hidden">
              <h2 class="text-base font-bold text-white tracking-tight">Certificate Preview</h2>
            </div>
            ...
            <div class="flex flex-wrap items-center justify-center gap-3 pt-2 print:hidden">
              <button
                ...
                class="px-5 py-2.5 rounded-2xl bg-slate-900 hover:bg-slate-800 text-white font-bold text-xs border border-slate-700 flex items-center gap-2 shadow-md transition-colors cursor-pointer"
              >
                <span>📥</span>
          ...
          <div class="bg-[#0F172A]/90 border border-slate-800/80 rounded-3xl p-5 shadow-xl space-y-2 print:hidden">
            ...
            <div class="text-xs text-slate-300 font-mono pt-1 flex flex-wrap gap-4">
              <span>Email: <a href="mailto:support@spilms.tech" class="text-purple-400 hover:underline">support@spilms.tech</a></span>
              <span>Phone: <a href="tel:+85512345678" class="text-purple-400 hover:underline">+855 12 345 678</a></span>
            </div>
          </div>
        ...
        <div class="lg:col-span-4 space-y-6 print:hidden">
          ...
          <div class="bg-[#0F172A]/90 border border-emerald-500/40 rounded-3xl p-6 shadow-2xl space-y-3 relative overflow-hidden">
            ...
            <div class="border-b border-slate-800/60 pb-2.5">
              <h3 class="text-xs font-bold text-slate-400 uppercase tracking-wider">Verification Status</h3>
            </div>

            <div class="flex items-center gap-4 pt-1">
              <div class="w-14 h-14 rounded-2xl bg-emerald-500/20 border border-emerald-500/40 text-emerald-400 flex items-center justify-center text-2xl shadow-lg shadow-emerald-950/50 shrink-0">
                🛡️
              </div>
              <div class="space-y-0.5">
                <h4 class="text-base font-black text-emerald-400 tracking-wide">VALID CERTIFICATE</h4>
                <p class="text-xs text-slate-300">This certificate is authentic and valid.</p>
                <p class="text-[10px] text-slate-400 font-mono pt-0.5">Verified on {{ currentResult.verified_on }}</p>
              </div>
            </div>
          </div>
          ...
          <div class="bg-[#0F172A]/90 border border-slate-800/80 rounded-3xl p-5 shadow-xl space-y-3">
            <div class="border-b border-slate-800/60 pb-2.5">
              <h3 class="text-sm font-bold text-white tracking-tight">Certificate Information</h3>
            </div>
            ...
            <div class="space-y-2.5 text-xs">
              ...
              <div class="flex items-center justify-between">
                ...
                <span class="font-bold text-white font-mono text-[11px]">{{ currentResult.certificate_id }}</span>
              </div>
              ...
              <div class="flex items-center justify-between">
                ...
                <span class="font-bold text-white">{{ currentResult.student_name }}</span>
              </div>
              ...
              <div class="flex items-center justify-between">
                ...
                <span class="font-bold text-white truncate max-w-[170px] text-right">{{ currentResult.course_name }}</span>
              </div>
              ...
              <div class="flex items-center justify-between">
                ...
                <span class="font-bold text-white">{{ currentResult.issuer }}</span>
              </div>
              ...
              <div class="flex items-center justify-between">
                ...
                <span class="font-bold text-white font-mono text-[11px]">{{ currentResult.completion_date }}</span>
              </div>
              ...
              <div class="flex items-center justify-between">
                ...
                <span class="font-bold text-white font-mono text-[11px]">{{ currentResult.issue_date }}</span>
              </div>
              ...
              <div class="flex items-center justify-between">
                ...
                <span class="font-bold text-white font-mono text-[11px]">{{ currentResult.verification_count }} times</span>
              </div>
          ...
          <div class="bg-[#0F172A]/90 border border-slate-800/80 rounded-3xl p-5 shadow-xl space-y-3">
            <div class="border-b border-slate-800/60 pb-2.5">
              <h3 class="text-sm font-bold text-white tracking-tight">Verification Features</h3>
            </div>
            ...
            <div class="space-y-3 text-xs">
              ...
              <div class="flex items-start gap-3">
                <div class="w-8 h-8 rounded-xl bg-emerald-600/20 border border-emerald-500/30 text-emerald-300 flex items-center justify-center text-sm shrink-0">
                  🛡️
                </div>
                <div>
                  <h4 class="font-bold text-white">Tamper Proof</h4>
                  <p class="text-[11px] text-slate-400 leading-tight mt-0.5">This certificate is digitally signed and tamper-proof</p>
                </div>
              </div>

              <!-- Feature 2 -->
              <div class="flex items-start gap-3">
                <div class="w-8 h-8 rounded-xl bg-blue-600/20 border border-blue-500/30 text-blue-300 flex items-center justify-center text-sm shrink-0">
                  🔒
                </div>
                <div>
                  <h4 class="font-bold text-white">Secure Verification</h4>
                  <p class="text-[11px] text-slate-400 leading-tight mt-0.5">Verified using blockchain technology</p>
                </div>
              </div>

              <!-- Feature 3 -->
              <div class="flex items-start gap-3">
                <div class="w-8 h-8 rounded-xl bg-purple-600/20 border border-purple-500/30 text-purple-300 flex items-center justify-center text-sm shrink-0">
                  🌐
                </div>
                <div>
                  <h4 class="font-bold text-white">Global Recognition</h4>
                  <p class="text-[11px] text-slate-400 leading-tight mt-0.5">Accepted and recognized worldwide</p>
                </div>
              </div>

              <!-- Feature 4 -->
              <div class="flex items-start gap-3">
                <div class="w-8 h-8 rounded-xl bg-amber-600/20 border border-amber-500/30 text-amber-300 flex items-center justify-center text-sm shrink-0">
                  ⚡
                </div>
                <div>
                  <h4 class="font-bold text-white">Instant Verification</h4>
                  <p class="text-[11px] text-slate-400 leading-tight mt-0.5">Real-time certificate authenticity check</p>
                </div>
              </div>
      ...
      <div class="bg-slate-900 border border-purple-500/40 rounded-3xl max-w-md w-full p-6 space-y-5 shadow-2xl relative">
        <div class="flex items-center justify-between border-b border-slate-800 pb-3">
          <div class="flex items-center gap-2">
            ...
            <h3 class="text-base font-bold text-white">Share Certificate</h3>
          </div>
          <button
        ...
        <div class="space-y-3 text-xs">
          <div>
            <span class="text-slate-400 text-[10px] uppercase font-bold">Certificate Name</span>
            <p class="text-sm font-bold text-white">{{ currentResult.course_name }}</p>
          </div>

          <div>
            <span class="text-slate-400 text-[10px] uppercase font-bold">Certificate ID</span>
            <p class="text-xs font-mono font-bold text-purple-300">{{ currentResult.certificate_id }}</p>
          </div>

          <!-- Copy Link -->
          <div class="space-y-1">
            <span class="text-slate-400 text-[10px] uppercase font-bold">Public Verification Link</span>
            <div class="flex items-center gap-2">
              <input
                type="text"
                readonly
                :value="currentResult.verification_url"
                class="flex-1 bg-slate-950 border border-slate-800 rounded-xl px-3 py-2 text-[11px] text-slate-300 font-mono"
              />
              <button
                @click="copyVerificationLink"
                class="px-3 py-2 rounded-xl bg-purple-600 hover:bg-purple-500 text-white font-bold text-xs whitespace-nowrap"
              >
                Copy Link
              </button>
            </div>
          ...
          <!-- Social Shortcuts -->
          <div class="pt-2">
            <span class="text-slate-400 text-[10px] uppercase font-bold block mb-2">Share on Social Media:</span>
            <div class="grid grid-cols-3 gap-2">
              <button
                @click="copyVerificationLink"
                class="py-2 px-3 rounded-xl bg-[#0088cc]/20 border border-[#0088cc]/40 hover:bg-[#0088cc]/30 text-sky-300 font-bold text-xs flex items-center justify-center gap-1.5"
              >
                <span>✈️</span> Telegram
              </button>
              <button
                @click="copyVerificationLink"
                class="py-2 px-3 rounded-xl bg-[#1877f2]/20 border border-[#1877f2]/40 hover:bg-[#1877f2]/30 text-blue-300 font-bold text-xs flex items-center justify-center gap-1.5"
              >
                <span>📘</span> Facebook
              </button>
              <button
                @click="copyVerificationLink"
                class="py-2 px-3 rounded-xl bg-purple-500/20 border border-purple-500/40 hover:bg-purple-500/30 text-purple-300 font-bold text-xs flex items-center justify-center gap-1.5"
              >
                <span>✉️</span> Email
              </button>
            </div>
        ...
        <div class="pt-2 border-t border-slate-800 flex justify-end">
          <button
            @click="isShareModalOpen = false"
            class="px-5 py-2 rounded-xl bg-slate-800 hover:bg-slate-700 text-slate-300 font-bold text-xs"
          >
            Close
          </button>
        </div>
