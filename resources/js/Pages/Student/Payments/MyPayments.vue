...
const confirmPayment = () => {
  ...
  setTimeout(() => {
    ...
    if (selectedInvoice.value) {
      ...
      selectedInvoice.value.receipt_number = 'REC-2025-' + selectedInvoice.value.id
    }
  }, 1000)
}
</script>
...
<template>
  <StudentLayout title="Course Fees & Invoices — Payment & Billing">
    <div class="space-y-6 max-w-7xl mx-auto pb-12">
      ...
      <div class="grid grid-cols-2 sm:grid-cols-3 lg:grid-cols-5 gap-4">
        ...
        <div class="bg-white dark:bg-[#0F172A]/90 border border-slate-200/90 dark:border-slate-800/80 rounded-2xl p-4 shadow-sm dark:shadow-xl flex items-center justify-between group hover:border-purple-500/30 transition-all">
          <div class="space-y-0.5">
            <p class="text-[10px] text-slate-500 dark:text-slate-400 font-medium">Total Due</p>
            <p class="text-2xl font-black text-slate-900 dark:text-white font-mono">{{ summary.total_due }}</p>
            <p class="text-[10px] text-slate-400 dark:text-slate-500 font-medium">{{ summary.total_due_note }}</p>
          </div>
        ...
        <div class="bg-white dark:bg-[#0F172A]/90 border border-slate-200/90 dark:border-slate-800/80 rounded-2xl p-4 shadow-sm dark:shadow-xl flex items-center justify-between group hover:border-blue-500/30 transition-all">
          <div class="space-y-0.5">
            <p class="text-[10px] text-slate-500 dark:text-slate-400 font-medium">Paid Amount</p>
            <p class="text-2xl font-black text-slate-900 dark:text-white font-mono">{{ summary.paid_amount }}</p>
            <p class="text-[10px] text-blue-600 dark:text-blue-400 font-medium font-mono">{{ summary.paid_amount_note }}</p>
          </div>
        ...
        <div class="bg-white dark:bg-[#0F172A]/90 border border-slate-200/90 dark:border-slate-800/80 rounded-2xl p-4 shadow-sm dark:shadow-xl flex items-center justify-between group hover:border-emerald-500/30 transition-all">
          <div class="space-y-0.5">
            <p class="text-[10px] text-slate-500 dark:text-slate-400 font-medium">Total Invoices</p>
            <p class="text-2xl font-black text-slate-900 dark:text-white font-mono">{{ summary.total_invoices }}</p>
            <p class="text-[10px] text-slate-400 dark:text-slate-500 font-medium">{{ summary.total_inv_note }}</p>
          </div>
        ...
        <div class="bg-white dark:bg-[#0F172A]/90 border border-slate-200/90 dark:border-slate-800/80 rounded-2xl p-4 shadow-sm dark:shadow-xl flex items-center justify-between group hover:border-emerald-500/30 transition-all">
          <div class="space-y-0.5">
            <p class="text-[10px] text-slate-500 dark:text-slate-400 font-medium">Paid Invoices</p>
            <p class="text-2xl font-black text-slate-900 dark:text-white font-mono">{{ summary.paid_invoices }}</p>
            <p class="text-[10px] text-emerald-600 dark:text-emerald-400 font-medium font-mono">{{ summary.paid_inv_note }}</p>
          </div>
          <div class="w-10 h-10 rounded-2xl bg-emerald-500/10 dark:bg-emerald-600/20 border border-emerald-500/20 dark:border-emerald-500/30 text-emerald-600 dark:text-emerald-300 flex items-center justify-center text-base shadow-inner shrink-0 group-hover:scale-110 transition-transform">
            ✓
          </div>
        ...
        <div class="bg-white dark:bg-[#0F172A]/90 border border-slate-200/90 dark:border-slate-800/80 rounded-2xl p-4 shadow-sm dark:shadow-xl flex items-center justify-between group hover:border-amber-500/30 transition-all">
          <div class="space-y-0.5">
            <p class="text-[10px] text-slate-500 dark:text-slate-400 font-medium">Pending Invoices</p>
            <p class="text-2xl font-black text-slate-900 dark:text-white font-mono">{{ summary.pending_invoices }}</p>
            <p class="text-[10px] text-amber-600 dark:text-amber-400 font-medium font-mono">{{ summary.pending_inv_note }}</p>
          </div>
      ...
      <div class="grid grid-cols-1 lg:grid-cols-12 gap-6">
        ...
        <div class="lg:col-span-8 space-y-6">
          ...
          <div class="flex flex-wrap items-center justify-between gap-3 bg-white dark:bg-[#0F172A]/80 border border-slate-200/90 dark:border-slate-800/80 rounded-2xl p-2.5 shadow-sm dark:shadow-lg">
            ...
            <div class="flex flex-wrap items-center gap-2">
              ...
              <button
                class="bg-slate-50 dark:bg-slate-900 border border-slate-200 dark:border-slate-700/80 text-xs font-semibold text-slate-700 dark:text-slate-300 rounded-xl px-3 py-1.5 hover:text-slate-950 dark:hover:text-white flex items-center gap-1.5 transition-colors cursor-pointer shadow-xs"
              >
                <span>📅</span>
          ...
          <div class="bg-white dark:bg-[#0F172A]/90 border border-slate-200/90 dark:border-slate-800/80 rounded-3xl p-5 shadow-sm dark:shadow-xl space-y-4">
            <h3 class="text-sm font-bold text-slate-900 dark:text-white tracking-tight">Invoices</h3>
            ...
            <div class="overflow-x-auto">
              <table class="w-full text-left text-xs">
                ...
                <tbody class="divide-y divide-slate-100 dark:divide-slate-800/60 font-medium">
                  ...
                  >
                    <!-- Invoice # -->
                    <td class="p-3 font-mono text-slate-700 dark:text-slate-300 font-bold whitespace-nowrap">
                      {{ inv.invoice_number }}
                    </td>

                    <!-- Course -->
                    <td class="p-3 font-bold text-slate-900 dark:text-white max-w-[160px] truncate">
                      {{ inv.course_name }}
                    </td>
                    ...
                    <!-- Amount -->
                    <td class="p-3 font-bold text-slate-900 dark:text-white font-mono">
                      {{ inv.amount }}
                    </td>
                    ...
                    <td class="p-3">
                      <span
                        :class="[
                          inv.status_type === 'paid' ? 'bg-emerald-500/10 dark:bg-emerald-500/20 text-emerald-700 dark:text-emerald-300 border-emerald-500/30' :
                          inv.status_type === 'overdue' ? 'bg-rose-500/10 dark:bg-rose-500/20 text-rose-700 dark:text-rose-300 border-rose-500/30' :
                    ...
                    <!-- Payment Method -->
                    <td class="p-3 text-slate-700 dark:text-slate-300 text-[11px]">
                      <div v-if="inv.payment_method !== '—'">
                        <span>{{ inv.payment_method }}</span>
                        <span v-if="inv.payment_method_sub" class="text-slate-400 dark:text-slate-500 font-mono block text-[9.5px]">{{ inv.payment_method_sub }}</span>
                      </div>
                    ...
                    <td class="p-3 text-right">
                      <div v-if="inv.status_type === 'paid'" class="flex items-center justify-end gap-1.5">
                        <button
                          ...
                          class="p-1.5 rounded-lg bg-slate-100 dark:bg-slate-900 hover:bg-slate-200 dark:hover:bg-slate-800 text-slate-600 dark:text-slate-400 hover:text-slate-950 dark:hover:text-white border border-slate-200 dark:border-slate-800 transition-colors shadow-xs"
                          title="View Invoice"
                        >
                          👁
                        </button>
                      ...
                      <div v-else class="flex items-center justify-end gap-1.5">
                        <button
                          ...
                          :class="[
                            inv.status_type === 'overdue'
                              ...
                              : 'bg-amber-600 hover:bg-amber-500 text-white shadow-md shadow-amber-950/20',
                        ...
                        <button
                          @click="openInvoiceModal(inv)"
                          class="p-1 rounded-lg text-slate-400 hover:text-slate-950 dark:hover:text-white"
                          title="More options"
                        >
                          ⋮
                        </button>
            ...
            <div class="pt-3 border-t border-slate-100 dark:border-slate-800/80 flex flex-col sm:flex-row items-center justify-between gap-3 text-xs">
              ...
              <div class="flex items-center gap-1.5">
                <button class="w-7 h-7 rounded-lg bg-slate-100 dark:bg-slate-900 border border-slate-200 dark:border-slate-800 text-slate-600 dark:text-slate-400 hover:text-slate-950 dark:hover:text-white flex items-center justify-center font-bold text-xs disabled:opacity-40 shadow-xs">
                  «
                </button>
          ...
          <div class="bg-gradient-to-r from-blue-50/70 via-white to-purple-50/70 dark:from-blue-950/70 dark:via-[#0F172A] dark:to-purple-950/70 border border-blue-100 dark:border-blue-900/50 rounded-3xl p-5 shadow-sm dark:shadow-xl flex flex-col md:flex-row items-center justify-between gap-4">
            <div class="space-y-3 flex-1">
              <div>
                <h4 class="text-sm font-bold text-slate-900 dark:text-white">Secure & Easy Payments</h4>
                <p class="text-[11px] text-slate-600 dark:text-slate-400 mt-0.5">Your payments are securely processed using industry-standard encryption.</p>
              </div>

              <div class="grid grid-cols-1 sm:grid-cols-3 gap-3 pt-1 text-xs">
                <div class="flex items-center gap-2">
                  <div class="w-7 h-7 rounded-xl bg-emerald-500/10 dark:bg-emerald-600/20 border border-emerald-500/20 dark:border-emerald-500/30 text-emerald-700 dark:text-emerald-300 flex items-center justify-center text-xs shrink-0">
                    🛡️
                  </div>
                  <div>
                    <p class="font-bold text-slate-900 dark:text-white text-[11px]">100% Secure</p>
                    <p class="text-[9.5px] text-slate-500 dark:text-slate-400">Your data is protected</p>
                  </div>
                </div>

                <div class="flex items-center gap-2">
                  <div class="w-7 h-7 rounded-xl bg-blue-500/10 dark:bg-blue-600/20 border border-blue-500/20 dark:border-blue-500/30 text-blue-700 dark:text-blue-300 flex items-center justify-center text-xs shrink-0">
                    💳
                  </div>
                  <div>
                    <p class="font-bold text-slate-900 dark:text-white text-[11px]">Multiple Payment Methods</p>
                    <p class="text-[9.5px] text-slate-500 dark:text-slate-400">ABA, Wing, Visa, Mastercard & more</p>
                  </div>
                </div>

                <div class="flex items-center gap-2">
                  <div class="w-7 h-7 rounded-xl bg-purple-500/10 dark:bg-purple-600/20 border border-purple-500/20 dark:border-purple-500/30 text-purple-700 dark:text-purple-300 flex items-center justify-center text-xs shrink-0">
                    ⚡
                  </div>
                  <div>
                    <p class="font-bold text-slate-900 dark:text-white text-[11px]">Instant Confirmation</p>
                    <p class="text-[9.5px] text-slate-500 dark:text-slate-400">Get payment receipt instantly</p>
                  </div>
                </div>
              </div>
            </div>
        ...
        <div class="lg:col-span-4 space-y-6">
          ...
          <div class="bg-white dark:bg-[#0F172A]/90 border border-slate-200/90 dark:border-slate-800/80 rounded-3xl p-5 shadow-sm dark:shadow-xl space-y-4">
            <div class="flex items-center justify-between border-b border-slate-100 dark:border-slate-800/60 pb-3">
              <h3 class="text-sm font-bold text-slate-900 dark:text-white tracking-tight">Payment Summary</h3>
            </div>
            ...
            <div class="flex items-center justify-between gap-4">
              ...
              <div class="relative w-24 h-24 flex items-center justify-center shrink-0">
                ...
                <div class="absolute inset-0 flex flex-col items-center justify-center text-center">
                  <span class="text-xs font-black text-slate-900 dark:text-white font-mono leading-none">{{ paymentSummary.total_amount }}</span>
                  <span class="text-[7.5px] text-slate-500 dark:text-slate-400 mt-0.5 font-medium">All time</span>
                </div>
              </div>
              ...
              <div class="space-y-1.5 text-xs flex-1">
                ...
                >
                  ...
                  <span class="font-bold text-slate-900 dark:text-white font-mono">{{ item.amount }} ({{ item.percentage }}%)</span>
                </div>
              </div>
            </div>
          </div>
          ...
          <div class="bg-white dark:bg-[#0F172A]/90 border border-slate-200/90 dark:border-slate-800/80 rounded-3xl p-5 shadow-sm dark:shadow-xl space-y-3">
            <div class="flex items-center justify-between border-b border-slate-100 dark:border-slate-800/60 pb-2.5">
              <h3 class="text-sm font-bold text-slate-900 dark:text-white tracking-tight">Upcoming Payments</h3>
              <Link href="/student/payments/my-payments?status=pending" class="text-xs text-purple-600 dark:text-purple-400 font-bold hover:underline">
                View All
              </Link>
            </div>
            ...
            <div class="space-y-2.5 text-xs">
              <div
                ...
                :key="item.id"
                class="p-3 rounded-2xl bg-slate-50 dark:bg-slate-900/70 border border-slate-200 dark:border-slate-800/60 flex items-center justify-between gap-3 hover:border-purple-500/30 transition-all shadow-xs"
              >
                <div class="flex items-center gap-2.5 min-w-0">
                  <div
                    :class="[
                      item.color === 'rose' ? 'bg-rose-500/10 dark:bg-rose-600/20 text-rose-700 dark:text-rose-300 border-rose-500/30' : 'bg-amber-500/10 dark:bg-amber-600/20 text-amber-700 dark:text-amber-300 border-amber-500/30',
                  ...
                  <div class="min-w-0">
                    <p class="font-bold text-slate-900 dark:text-white truncate max-w-[130px]">{{ item.course }}</p>
                    <p class="text-[10px] text-slate-500 dark:text-slate-400">{{ item.due_date }}</p>
                  </div>
                </div>

                <div class="text-right">
                  <p class="font-bold text-slate-900 dark:text-white font-mono">{{ item.amount }}</p>
                  <span
                    :class="[
                      item.color === 'rose' ? 'text-rose-600 dark:text-rose-400' : 'text-amber-600 dark:text-amber-400',
          ...
          <div class="bg-white dark:bg-[#0F172A]/90 border border-slate-200/90 dark:border-slate-800/80 rounded-3xl p-5 shadow-sm dark:shadow-xl space-y-3">
            <div class="flex items-center justify-between border-b border-slate-100 dark:border-slate-800/60 pb-2.5">
              <h3 class="text-sm font-bold text-slate-900 dark:text-white tracking-tight">Recent Transactions</h3>
              <Link href="/student/payments/transactions" class="text-xs text-purple-600 dark:text-purple-400 font-bold hover:underline">
                View All
              </Link>
            </div>
            ...
            <div class="space-y-2.5 text-xs">
              <div
                ...
                :key="tx.id"
                class="p-2.5 rounded-2xl bg-slate-50 dark:bg-slate-900/70 border border-slate-200 dark:border-slate-800/60 flex items-center justify-between gap-3 shadow-xs"
              >
                <div class="flex items-center gap-2.5 min-w-0">
                  <div
                    :class="[
                      tx.amount_type === 'positive' ? 'bg-emerald-500/10 dark:bg-emerald-600/20 text-emerald-700 dark:text-emerald-300 border-emerald-500/30' : 'bg-amber-500/10 dark:bg-amber-600/20 text-amber-700 dark:text-amber-300 border-amber-500/30',
                      'w-7 h-7 rounded-xl flex items-center justify-center text-xs shrink-0 border'
                    ]"
                  >
                  ...
                  <div class="min-w-0">
                    <p class="font-bold text-slate-900 dark:text-white truncate">{{ tx.title }}</p>
                    <p class="text-[9.5px] text-slate-500 dark:text-slate-400 font-mono">{{ tx.date }}</p>
                  </div>
                </div>
                ...
                <div class="text-right">
                  <p
                    :class="[
                      tx.amount_type === 'positive' ? 'text-emerald-600 dark:text-emerald-400' : 'text-amber-600 dark:text-amber-400',
                      'font-bold font-mono text-xs'
                    ]"
                  >
                    {{ tx.amount }}
                  </p>
                  <p class="text-[9.5px] text-slate-400 dark:text-slate-500 font-mono">{{ tx.invoice_number }}</p>
                </div>
              </div>
            </div>
          </div>
          ...
          <!-- WIDGET 4: NEED HELP? -->
          <div class="bg-gradient-to-br from-indigo-50/80 via-white to-purple-50/60 dark:from-[#10132B] dark:via-[#0F172A] dark:to-[#1E1138] border border-purple-200 dark:border-purple-900/50 rounded-3xl p-5 shadow-sm dark:shadow-2xl space-y-3.5">
            <div class="flex items-center justify-between">
              <div>
                <h3 class="text-sm font-bold text-slate-900 dark:text-white">Need Help?</h3>
                <p class="text-xs text-slate-600 dark:text-slate-400 mt-0.5">Having issues with payments or invoices? Our support team is here to help you.</p>
              </div>
              <div class="w-12 h-12 rounded-2xl bg-purple-500/10 dark:bg-purple-600/20 border border-purple-500/20 dark:border-purple-500/30 text-purple-600 dark:text-purple-300 flex items-center justify-center text-xl shrink-0">
                🎧
              </div>
            </div>
            ...
            <a
              ...
              class="w-full py-2.5 rounded-xl bg-purple-600 hover:bg-purple-500 text-white font-bold text-xs shadow-lg shadow-purple-950/20 flex items-center justify-center gap-2 transition-colors cursor-pointer"
            >
              <span>🎧</span>
    ...
    >
      <div class="bg-white dark:bg-slate-900 border border-slate-200 dark:border-purple-500/40 rounded-3xl max-w-lg w-full p-6 space-y-5 shadow-2xl relative">
        <div class="flex items-center justify-between border-b border-slate-100 dark:border-slate-800 pb-3">
          <div>
            <h3 class="text-base font-black text-slate-900 dark:text-white">Invoice Details</h3>
            <p class="text-xs font-mono text-purple-600 dark:text-purple-300">{{ selectedInvoice.invoice_number }}</p>
          </div>
          <button
        ...
        <div class="p-4 bg-slate-50 dark:bg-slate-950 rounded-2xl border border-slate-200 dark:border-slate-800 space-y-3 text-xs">
          <div class="flex justify-between border-b border-slate-200 dark:border-slate-800/80 pb-2">
            <span class="text-slate-600 dark:text-slate-400">Course / Program:</span>
            <span class="font-bold text-slate-900 dark:text-white">{{ selectedInvoice.course_name }}</span>
          </div>
          <div class="flex justify-between border-b border-slate-200 dark:border-slate-800/80 pb-2">
            <span class="text-slate-600 dark:text-slate-400">Invoice Date:</span>
            <span class="font-mono text-slate-700 dark:text-slate-300">{{ selectedInvoice.invoice_date }}</span>
          </div>
          <div class="flex justify-between border-b border-slate-200 dark:border-slate-800/80 pb-2">
            <span class="text-slate-600 dark:text-slate-400">Due Date:</span>
            <span class="font-mono text-slate-700 dark:text-slate-300">{{ selectedInvoice.due_date }}</span>
          </div>
          <div class="flex justify-between border-b border-slate-200 dark:border-slate-800/80 pb-2">
            <span class="text-slate-600 dark:text-slate-400">Status:</span>
            <span
              :class="[
                selectedInvoice.status_type === 'paid' ? 'text-emerald-600 dark:text-emerald-400' : 'text-amber-600 dark:text-amber-400',
                'font-bold uppercase'
              ]"
            >
          ...
          <div class="flex justify-between pt-1">
            <span class="text-slate-700 dark:text-slate-300 font-bold">Total Amount Due:</span>
            <span class="text-base font-black text-slate-900 dark:text-white font-mono">{{ selectedInvoice.amount }}</span>
          </div>
        </div>
        ...
        <div class="flex items-center justify-between pt-2 border-t border-slate-100 dark:border-slate-800 text-xs">
          ...
          <div class="flex items-center gap-2">
            <button
              ...
              class="px-4 py-2 rounded-xl bg-slate-100 dark:bg-slate-800 hover:bg-slate-200 dark:hover:bg-slate-700 text-slate-900 dark:text-white font-bold border border-slate-200 dark:border-slate-700 cursor-pointer"
            >
              Download Receipt
            </button>
            <button
    ...
    >
      <div class="bg-white dark:bg-slate-900 border border-slate-200 dark:border-purple-500/40 rounded-3xl max-w-md w-full p-6 space-y-4 shadow-2xl relative">
        ...
        <div class="flex items-center justify-between border-b border-slate-100 dark:border-slate-800 pb-3">
          <div>
            <h3 class="text-base font-black text-slate-900 dark:text-white">Complete Your Payment</h3>
            <p class="text-xs text-slate-500 dark:text-slate-400 font-mono">{{ selectedInvoice.invoice_number }} • {{ selectedInvoice.amount }}</p>
          </div>
          <button
        ...
        <div v-if="isPaymentProcessing" class="py-10 text-center space-y-3">
          ...
          <h4 class="text-sm font-bold text-slate-900 dark:text-white">Processing your payment securely...</h4>
          <p class="text-xs text-slate-500 dark:text-slate-400">Verifying KHQR transaction and bank callback</p>
        </div>

        <!-- STATE B: SUCCESS STATE -->
        <div v-else-if="isPaymentSuccess" class="py-6 text-center space-y-4">
          <div class="w-14 h-14 rounded-full bg-emerald-500/10 dark:bg-emerald-500/20 border border-emerald-500/30 text-emerald-600 dark:text-emerald-400 flex items-center justify-center text-3xl mx-auto shadow-lg shadow-emerald-950/20">
            ✓
          </div>
          <div>
            <h4 class="text-base font-black text-emerald-600 dark:text-emerald-400">Payment Successful</h4>
            <p class="text-xs text-slate-600 dark:text-slate-300 mt-1">Your payment of {{ selectedInvoice.amount }} has been completed.</p>
          </div>

          <div class="p-3 bg-slate-50 dark:bg-slate-950 rounded-xl border border-slate-200 dark:border-slate-800 text-xs font-mono text-left space-y-1">
            <div class="flex justify-between"><span class="text-slate-500">Method:</span><span class="text-slate-700 dark:text-slate-300">{{ selectedInvoice.payment_method }}</span></div>
            <div class="flex justify-between"><span class="text-slate-500">TXN ID:</span><span class="text-slate-700 dark:text-slate-300">{{ selectedInvoice.transaction_id }}</span></div>
          </div>

          <button
            @click="isPaymentModalOpen = false"
            class="w-full py-2.5 rounded-xl bg-emerald-600 hover:bg-emerald-500 text-white font-bold text-xs shadow-md cursor-pointer"
          >
            Done
          </button>
        </div>
        ...
        <div v-else class="space-y-4">
          ...
          <div class="p-3 bg-slate-50 dark:bg-slate-950 rounded-2xl border border-slate-200 dark:border-slate-800 text-xs space-y-1">
            <div class="flex justify-between"><span class="text-slate-500 dark:text-slate-400">Course:</span><span class="font-bold text-slate-900 dark:text-white">{{ selectedInvoice.course_name }}</span></div>
            <div class="flex justify-between"><span class="text-slate-500 dark:text-slate-400">Amount Due:</span><span class="font-bold text-slate-900 dark:text-white font-mono">{{ selectedInvoice.amount }}</span></div>
          </div>

          <!-- Method Selection -->
          <div class="space-y-2">
            <p class="text-xs font-bold text-slate-900 dark:text-white uppercase tracking-wider">Select Payment Method:</p>
            ...
            <div class="grid grid-cols-2 gap-2 text-xs">
              <button
                ...
                :class="[
                  selectedPaymentMethod === 'aba'
                    ? 'bg-purple-50 dark:bg-purple-600/20 border-purple-500 text-purple-900 dark:text-white shadow-md font-bold'
              ...
              >
                ...
                <span class="text-[9px] text-emerald-600 dark:text-emerald-400 font-mono">Scan & Pay</span>
              </button>
              ...
              >
                ...
                <span class="text-[9px] text-blue-600 dark:text-blue-400 font-mono">Instant Pay</span>
              </button>
          ...
          <div v-if="selectedPaymentMethod === 'aba'" class="p-3 bg-slate-50 dark:bg-slate-950 rounded-2xl border border-slate-200 dark:border-slate-800 text-center space-y-2">
            ...
            <p class="text-[10px] text-slate-600 dark:text-slate-400">Scan with any Bakong or Banking App to pay <strong>{{ selectedInvoice.amount }}</strong></p>
          </div>

          <!-- Action -->
          <button
            @click="confirmPayment"
            class="w-full py-2.5 rounded-2xl bg-gradient-to-r from-purple-600 to-indigo-600 hover:from-purple-500 hover:to-indigo-500 text-white font-bold text-xs shadow-lg shadow-purple-950/20 transition-all cursor-pointer active:scale-95"
          >
